...
axios.get('https://amanacart.com/api/admin/catalog/attribute', auth).then(response => {
  data = response.data
  // console.log(data)
}).catch(error => {
        // console.error(error)
...
mock.onGet('/api/attribute/initial-data').reply(config => {
  return [200, data]
})

mock.onGet('/api/attribute/data').reply(config => {
