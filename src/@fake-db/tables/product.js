...
axios.get('https://amanacart.com/api/admin/catalog/product', auth).then(response => {
  data = response.data
  // console.log(data)
}).catch(error => {
        // console.error(error)
...
mock.onGet('/api/product/initial-data').reply(config => {
  return [200, data]
})

mock.onGet('/api/product/data').reply(config => {
